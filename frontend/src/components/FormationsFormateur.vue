<template>
    <v-container class=" col-12 px-7" fluid>
      <v-data-table :headers="headers" :items="filterFormationsByUserId" sort-by="id" class="elevation-1" :search="search">
        <template v-slot:top>
          <v-toolbar flat>
        ...
        <template v-slot:no-data>
          <v-btn color="primary" @click="filterFormationsByUserId">
            Réinitialiser
          </v-btn>
        </template>
      </v-data-table>
  ...
  export default {
    data() {
      return {
        ...
        headers: [
          ...
          // { text: "Code Commentaire", value: "code_commentaire" },
        ],
        editedIndex: -1,
    ...
    computed: {
      ...mapGetters(['formations','getFormationsByUserId','getAccessToken', 'getCurrentUserId']),
      formTitle() {
      ...
      filterFormationsByUserId() {
        const userId = this.getCurrentUserId;
        return this.getFormationsByUserId(userId);
      },
    },
  
    methods: {
      ...mapActions(['createFormation', 'updateFormation', 'deleteFormation', 'createAxios']),
      createEmptyItem() {
        return {
          id: "",
          type_structure: "",
  ...
  async deleteItemConfirm() {
      try {
        await this.deleteFormation({
          ...
          closeDialog: this.closeDialog, accessToken: this.getAccessToken, createAxiosAction: "createAxios",
        });
      } catch (error) {
        console.error(error);
      }
      ...
      async save() {
        ...
        if (this.editedIndex > -1) {
          await this.updateFormation({editedItemId: this.editedItem.id, editedItem: this.editedItem , accessToken: this.getAccessToken, createAxiosAction: "createAxios", });
        } else {
          this.editedItem.user_id = this.getCurrentUserId;
           await this.createFormation({ editedItem: this.editedItem , accessToken: this.getAccessToken, createAxiosAction: "createAxios",});
        }
